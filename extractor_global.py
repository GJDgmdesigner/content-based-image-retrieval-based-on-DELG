...
MODEL_WEIGHTS = '/Data_HDD/staff_xi_yang/LYMDataset/output2/checkpoints/model_epoch_0030.pyth'
PATH = "extraction/f43b9af15990f7de.jpg"
INFER_DIR = '/Data_HDD/yiming/datasets/rparis6k/jpg'
COMBINE_DIR = "/Data_HDD/yiming/gjd/"
...
def delg_extract(img, model):
    ...
    for scale_factor in SCALE_LIST:
        ...
        #tmp = delg_scores.squeeze().reshape(-1)
...
def main(spath):
    ...
    for index, imgfile in enumerate(walkfile(spath)):
        ...
        if ext.lower() in ['.jpg', '.jpeg', '.bmp', '.png', '.pgm']:
            ...
            if im is None:
                print("{} is not readable".format(imgfile))
                continue
...
def main_multicard(spath, cutno, total_num):
    ...
    for index, imgfile in enumerate(walkfile(spath)):
        ...
        if ext.lower() in ['.jpg', '.jpeg', '.bmp', '.png', '.pgm']:
            ...
            im = im.astype(np.float32, copy=False)
            data =  delg_extract(im, model)
            print(data['locations'].shape, data['descriptors'].shape)       #(1000, 2), (1000, 512)
            feadic[name] = data
