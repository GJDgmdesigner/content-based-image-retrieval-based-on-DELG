...
def train_epoch(train_loader, model, loss_fun, optimizer, train_meter, cur_epoch):
    ...
    for cur_iter, (inputs, labels) in enumerate(train_loader):
        ...
        # Compute the loss
        desc_loss = loss_fun(global_logits, labels)          #计算全局特征提取的loss(损失函数，预测结果和实际结果之间的差别)
        att_loss = loss_fun(local_logits, labels)            #计算局部特征提取的loss
        # Perform the backward pass
        optimizer.zero_grad()                                #梯度清零
        # Freeze localmodel
        net.freeze_weights(model, freeze=['localmodel', 'att_cls'])   #冻结局部特征的权重
        desc_loss.backward()                                 #反向传播全局loss
        #desc_loss.backward(retain_graph=True)

        # update params
        #optimizer.step()

        # Freeze globalmodel and unfreeze localmodel
        net.freeze_weights(model, freeze=['globalmodel', 'desc_cls'])       #冻结全局特征提取的loss
        net.unfreeze_weights(model, freeze=['localmodel', 'att_cls'])       #解冻局部特征提取的loss
        att_loss.backward()                                                 #反向传播局部loss
        #att_loss.backward(retain_graph=True)

        net.unfreeze_weights(model, freeze=['globalmodel', 'desc_cls'])     #冻结全局特征提取的loss
