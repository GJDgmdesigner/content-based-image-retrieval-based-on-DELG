...
from model.resnet import ResHead

from core.builders import get_model

class Delg(nn.Module):
    def __init__(self):
        super(Delg, self).__init__()
        self.globalmodel = get_model()()
        self.desc_cls = Arcface(cfg.MODEL.HEADS.REDUCTION_DIM, cfg.MODEL.NUM_CLASSES)
        self.localmodel = SpatialAttention2d(cfg.MODEL.HEADS.LOCAL_IN_FEAT)
...
class Arcface(nn.Module):
    ...
    def forward(self, features, targets):
        ...
        target_logit = cos_theta[torch.arange(0, features.size(0)), targets].reshape(-1, 1)
        ...
        cos_theta.scatter_(1, targets.reshape(-1, 1).long(), final_target_logit)
