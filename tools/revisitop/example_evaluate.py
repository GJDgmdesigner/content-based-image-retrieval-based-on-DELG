...
# data_root = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'data')    # edit5
data_root = "/Data_HDD/yiming/gjd"
...
# cfg = configdataset(test_dataset, os.path.join(data_root, 'datasets'))                    # edit7
cfg = configdataset(test_dataset, "/Data_HDD/yiming/datasets/rparis6k/")
...
with open("/Data_HDD/yiming/gjd/localfea.pickle_1", "rb") as f:
    features = pickle.load(f)
    print(">> local features loaded ...")
    
    for picture_name in features.keys():
        pict_dict = feature.get(picture_name)
        locations_info = feature.get("locations")                            # Q
        description_info = feature.get("descriptors")                        # X
        Q = Q.concat(locations_info)
        X = X.concat(description_info)
        # scores_info = feature.get("scores")                                 #all three infos are array object
